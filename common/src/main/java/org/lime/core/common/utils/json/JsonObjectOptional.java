...
public class JsonObjectOptional extends JsonElementOptional implements Map<String, JsonElementOptional> {
    ...
    @Override public Set<Entry<String, JsonElementOptional>> entrySet() { return this.members.entrySet(); }
    ...
    @Override public void putAll(@NotNull Map<? extends String, ? extends JsonElementOptional> map) { map.forEach(this::add); }
    ...
    public <T extends Enum<T>>Optional<T> getAsEnum(Class<T> tClass, String memberName) { return get(memberName).flatMap(v -> v.getAsEnum(tClass)); }
