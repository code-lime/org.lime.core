...
public class JavaScript implements ElementInstance {
    ...
    static {
        try {
            ...
            BaseCoreInstance.global.$logOP("A: " + Modifier.toString(field.getModifiers()));
            field = Reflection.nonFinal(field);
            BaseCoreInstance.global.$logOP("B: " + Modifier.toString(field.getModifiers()));
            Accessor.LanguageSupport base = (Accessor.LanguageSupport)field.get(null);
            BaseCoreInstance.global.$logOP("C: " + base);
            field = Reflection.access(field);
            field.set(null, LanguageAccessorProxy.create(base));
            BaseCoreInstance.global.$logOP("D");
    ...
    public static <T extends BaseCoreElement<JavaScript,?,?,T>>T create(Func1<Class<JavaScript>, T> create) {
        return new JavaScript().$create(create);
    }
    private <T extends BaseCoreElement<JavaScript,?,?,T>>T $create(Func1<Class<JavaScript>, T> create) {
        return create.invoke(JavaScript.class)
                .withInstance(this)
                .withInit(this::init)
    ...
    @Override public void core(BaseCoreInstance<?,?> baseCore) { this.baseCore = baseCore; }
    @Override public BaseCoreInstance<?,?> core() { return baseCore; }

    public static abstract class InstanceJS {
        public final ConcurrentLinkedQueue<ScheduleTask> tasks = new ConcurrentLinkedQueue<>();
    }
    ...
    private synchronized Object eval(String script, @Nullable Map<String, Object> values) throws Exception {
        try (var frame = context.frame()) {
            ...
            if (methodArgsLine.isEmpty()) return invokeFunction(engine, global, methodName);
    ...
    public void init() {
        instances.values().forEach(v -> v.tasks.removeIf(ScheduleTask::isCancelled));
    }
    public void uninit() {
        context.close();
    }
    public void reinstance(boolean cancel) {
        try (var frame = context.frame()) {
            ...
            if (cancel) instances.values().forEach(v -> v.tasks.forEach(ScheduleTask::cancel));
    ...
    public void config(String js) {
        ...
        for (String line : js.split("\n")) {
            if (_import && line.startsWith("//import ")) {
                ...
                baseCore.$logOP("JS import: " + module);
                _modules.put(module, baseCore.$readAllConfig(module, ".js"));
            } else {
                _js.add(line);
                _import = false;
            }
        }
        instances.values().forEach(v -> v.tasks.forEach(ScheduleTask::cancel));
        ...
        try (var frame = context.frame()) {
            ...
            baseCore.$logOP("JavaScriptEngine: " + engine.getClass().getName());
            String loadModule = "???";
            try {
                ...
                baseCore.$logOP("JS ERROR:" + loadModule + ".js");
                baseCore.$logStackTrace(e);
            }
            try {
                eval(String.join("\n", _js));
                eval("init()");
                ...
                baseCore.$logOP("JS ERROR:global.js");
                baseCore.$logStackTrace(e);
            }
        }
    }
    ...
    private record ArgsBinding(Map<String, Object> args, Bindings owner) implements Bindings {
        ...
        @Override public void putAll(Map<? extends String, ?> toMerge) { owner.putAll(toMerge); }
