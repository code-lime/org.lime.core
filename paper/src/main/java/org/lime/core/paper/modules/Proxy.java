...
public class Proxy implements ElementInstance {
    ...
    public static <T extends BaseCoreElement<Proxy,?,?,T>>T create(Func1<Class<Proxy>, T> create) {
        return new Proxy().$create(create);
    }
    private <T extends BaseCoreElement<Proxy,?,?,T>>T $create(Func1<Class<Proxy>, T> create) {
        return create.invoke(Proxy.class)
                .withInstance(this)
                .withInit(() -> {
                ...
                .withUninit(this::unregister);
    }

    private CoreInstancePlugin.CoreInstance baseCore;
    private Plugin plugin;

    @Override public void core(BaseCoreInstance<?,?> baseCore) {
        this.baseCore = CoreInstancePlugin.CoreInstance.class.cast(baseCore);
        this.plugin = this.baseCore.plugin();
    }
